...
function showSchedule_(ctx, dateStr) {
  ...
  appts.forEach(function (a) {
    var user = a.user_first_name + (a.user_last_name ? (' ' + a.user_last_name) : '');
    var tag = a.user_username ? (' @' + a.user_username) : '';
    text +=
      '🕐 *' + t_(a.start_time) + '–' + t_(a.end_time) + '* | ' + a.service_name + ' | ' + a.master_name + '\n' +
      '👤 ' + user + tag + ' · #' + a.id + '\n\n';
...
function adminActions_(ctx) {
  ...
  if (data.indexOf('admin:mastersvc:') === 0) {
    ...
    edit_(ctx, '💅 *Услуги: ' + master.name + '*', Object.assign({ parse_mode: 'Markdown' }, ik_(rows)));
    return;
  }
  ...
  if (data.indexOf('admin:master:') === 0) {
    ...
    var svcList = svcs.length ? svcs.map(function (s) { return s.name; }).join(', ') : 'не назначены';
