...
var BOOKING_SCENE = {
  steps: [
    ...
    function (ctx, session) {
      ...
      if (!dates.length) {
        ctx.editMessageText(
          '😔 У мастера *' + escMd_(master.name) + '* нет свободных дат на ближайшие 2 недели.',
          { parse_mode: 'Markdown' }
        );
        leaveScene_(ctx.from.id);
        return;
      ...
      ctx.editMessageText(
        '💅 *' + escMd_(session.state.serviceName) + '*\n👩 ' + escMd_(master.name) + '\n\nВыберите дату:',
    ...
    function (ctx, session) {
      ...
      ctx.editMessageText(
        '💅 *' + escMd_(session.state.serviceName) + '*\n👩 ' + escMd_(session.state.masterName) + '\n📅 ' + formatDateFull_(dateStr) + '\n\nВыберите время:',
        Object.assign({ parse_mode: 'Markdown' }, ik_(rows))
      );
      advanceScene_(ctx, session);
    },
    ...
    function (ctx, session) {
      ...
      ctx.editMessageText(
        '✅ *Подтвердите запись:*\n\n' +
        '💅 Услуга: *' + escMd_(s.serviceName) + '*\n' +
        '👩 Мастер: *' + escMd_(s.masterName) + '*\n' +
    ...
    function (ctx, session) {
      ...
      // Notify admins
      var userName = escMd_(ctx.from.first_name) + (ctx.from.last_name ? (' ' + escMd_(ctx.from.last_name)) : '');
      var tag = ctx.from.username ? (' (@' + escMd_(ctx.from.username) + ')') : '';
      var adminText =
