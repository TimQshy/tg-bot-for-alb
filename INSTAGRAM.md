...
Первое сообщение цепочки на комментарий адресуется по `comment_id`,
остальные — по id получателя: его берём из `recipient_id` в ответе API, а
если его там нет — из `from.id` вебхука.
